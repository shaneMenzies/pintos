...
struct key_action {
    ...
    inline key_action(key_action_codes code   = TO_BUFFER,
...
class kb_handler {
  ...
  public:
    ...
    inline void buffer_write_c(char target) {
        ...
        if (target == '\b') {
            if (buffer_next > buffer_start) buffer_next--;
            *buffer_next = '\0';
        } else {
            *buffer_next = target;
            buffer_next++;
        ...
        if ((uintptr_t)on_buffer_write != 0) { on_buffer_write(target); }
    }
    ...
    inline const char* get_buffer() const { return (const char*)buffer_start; }

    inline void set_action(char       target,
                           key_action action = key_action(TO_BUFFER, 0)) {
    ...
    inline void set_signal(unsigned char index,
                           void (*new_signal)(kb_handler* handler,
                                              char        character)) {
        signal[index] = new_signal;
    }

    inline void clear_signal(unsigned char index) { signal[index] = 0; }
