...
#define _STL_PAIR_H 1
    ...
    #include "../type_traits" // for std_k::__decay_and_strip
    #include "move.h"    // for std_k::move / std_k::forward, and std_k::swap
    #include "utility.h" // for std_k::tuple_element, std_k::tuple_size
#endif
...
#if __cplusplus >= 201103L
/// Tag type for piecewise construction of std_k::pair objects.
struct piecewise_construct_t {
    explicit piecewise_construct_t() = default;
};

/// Tag for piecewise construction of std_k::pair objects.
...
struct pair : public __pair_base<_T1, _T2> {
  ...
  public:
    ...
    constexpr explicit(!_S_convertible<_U1, _U2>())
        ...
        : first(std_k::forward<_U1>(__x))
        , second(std_k::forward<_U2>(__y)) {}
    ...
    constexpr explicit(!_S_convertible<_U1, _U2>()) pair(
        ...
        : first(std_k::forward<_U1>(__p.first))
        , second(std_k::forward<_U2>(__p.second)) {}
  ...
  public:
    ...
    {
        first  = std_k::forward<first_type>(__p.first);
        second = std_k::forward<second_type>(__p.second);
        return *this;
    }
    ...
    explicit constexpr pair(const pair<_U1, _U2>& __p)
        ...
        , second(__p.second) {}
                ...
                #define _GLIBCXX_DEPRECATED_PAIR_CTOR                         \
                    __attribute__((                                           \
                        __deprecated__("use 'nullptr' instead of '0' to "     \
                                       "initialize std_k::pair of move-only " \
                                       "type and pointer")))
            #else
                #define _GLIBCXX_DEPRECATED_PAIR_CTOR
            #endif
  ...
  public:
    ...
    template<typename _U1,
             __enable_if_t<__and_<__not_<is_reference<_U1>>, is_pointer<_T2>,
                           ...
                           bool>
             = true>
    _GLIBCXX_DEPRECATED_PAIR_CTOR constexpr pair(
    ...
    // 265.  std_k::pair::pair() effects overly restrictive
    /** The default constructor creates @c first and @c second using their
     *  respective default constructors.  */
    pair()
        : first()
...
#if __cplusplus >= 201103L
/** Swap overload for pairs. Calls std_k::pair::swap().
 *
 * @note This std_k::swap overload is not declared in C++03 mode,
...
constexpr pair<typename __decay_and_strip<_T1>::__type,
    ...
    return __pair_type(std_k::forward<_T1>(__x), std_k::forward<_T2>(__y));
}
#else
template<typename _T1, typename _T2>
...
#if __cplusplus >= 201103L
// Various functions which give std_k::pair a tuple-like interface.

template<typename _T1, typename _T2>
struct __is_tuple_like_impl<pair<_T1, _T2>> : true_type {};

/// Partial specialization for std_k::pair
...
template<> struct __pair_get<0> {
    ...
    static constexpr _Tp1&& __move_get(pair<_Tp1, _Tp2>&& __pair) noexcept {
        return std_k::forward<_Tp1>(__pair.first);
    }
    ...
    static constexpr const _Tp1&&
        ...
        return std_k::forward<const _Tp1>(__pair.first);
    }
};
...
template<> struct __pair_get<1> {
    ...
    static constexpr _Tp2&& __move_get(pair<_Tp1, _Tp2>&& __pair) noexcept {
        return std_k::forward<_Tp2>(__pair.second);
    }
    ...
    static constexpr const _Tp2&&
        __const_move_get(const pair<_Tp1, _Tp2>&& __pair) noexcept {
        return std_k::forward<const _Tp2>(__pair.second);
    }
};
/// @endcond

/** @{
 * std_k::get overloads for accessing members of std_k::pair
 */
...
constexpr typename tuple_element<_Int, pair<_Tp1, _Tp2>>::type&&
    get(pair<_Tp1, _Tp2>&& __in) noexcept {
    return __pair_get<_Int>::__move_get(std_k::move(__in));
}
...
constexpr const typename tuple_element<_Int, pair<_Tp1, _Tp2>>::type&&
    get(const pair<_Tp1, _Tp2>&& __in) noexcept {
    return __pair_get<_Int>::__const_move_get(std_k::move(__in));
}
