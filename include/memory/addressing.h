...
struct address_space {
    paging::page_level_4_table*                 primary_table;
    std_k::vector<paging::page_level_4_table*>* shared_tables = nullptr;
    ...
    address_space() {
        ...
        primary_table = new paging::page_level_4_table;

        // Initialize the bare minimum mapping in top 2 pdps
        paging::initialize_table_mapping(primary_table);

        // Copy over higher half of kernel mapping
        for (int i = 0; i < 254; i++) {
            primary_table->data[i + 256] = paging::pml4_table->data[i + 256];
        }
    }

    address_space(address_space* parent_space)
        : address_space() {
        shared_tables = parent_space->link_table(primary_table);
    }

    ~address_space() {
        // Unlink self from shared tables
        bool last_table = true;
        if (shared_tables != nullptr) {
            if (shared_tables->size() <= 1) {
                delete shared_tables;
            } else {
                last_table = false;
                for (size_t index = 0; index < shared_tables->size(); index++) {
                    if (shared_tables->at(index) == primary_table) {
                        shared_tables->erase(index);
                        break;
                    }
                }
            }
        ...
        if (last_table) {
            // TODO: Clean up tables from inside address space
            // Need to clean up tables
        }
    }

    std_k::vector<paging::page_level_4_table*>*
        link_table(paging::page_level_4_table* target) {
        if (shared_tables == nullptr) {
            // Create new vector
            shared_tables = new std_k::vector<paging::page_level_4_table*>;
            shared_tables->push_back(primary_table);
        }

        shared_tables->push_back(target);
    ...
    inline page_directory_pointer_table*
        ...
        bool exists   = (primary_table->data[l4_index] != 0);
        if (!exists) {
            ...
            if (l4_index < 255) {
                ...
                for (size_t i = 0; i < shared_tables->size(); i++) {
                    shared_tables->at(i)->data[l4_index] = (uintptr_t)p_address
                                                           | pdp_present
                                                           | pdp_write_enabled;
                }
            } else {
