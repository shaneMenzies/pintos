...
constexpr uint64_t RSDP_SIGNATURE = 0x2052545020445352;
...
struct table_header {
    ...
    bool valid_checksum() const {
        ...
        for (uint32_t i = 0; i < length; i++) { sum += ((char*)this)[i]; }
...
struct old_rsdp {
    ...
    rsdt* get_rsdt() const { return (rsdt*)(rsdt_addr | 0ULL); }
...
struct modern_rsdp : old_rsdp {
    ...
    xsdt* get_xsdt() const { return xsdt_addr; }
...
int get_entries(table_header* target_table, uint8_t target_type,
                entry_header** entry_buffer, int buffer_max = 32);
