...
void framebuffer_init(multiboot_boot_info* mb_info) {
    ...
    fb.pixel_size = mb_info->framebuffer_tag->common.framebuffer_pitch
                    / mb_info->framebuffer_tag->common.framebuffer_width;
    ...
    if (mb_info->framebuffer_tag->common.framebuffer_type == 1) {
        ...
        fb.green_shift
            = mb_info->framebuffer_tag->framebuffer_green_field_position;
        fb.green_mask = mb_info->framebuffer_tag->framebuffer_green_mask_size;
        fb.blue_shift
            = mb_info->framebuffer_tag->framebuffer_blue_field_position;
    ...
    paging::kernel_address_space.identity_map_region((uintptr_t)fb.address,
                                                     fb.size);
}
 ...
 */
void v_fb::ega_putc(uint32_t& x, uint32_t& y, uint8_t ega_attributes,
                    const char character) {
    if (fb_initialized && info.ega_text) {

        uint16_t* target_address
            = (uint16_t*)(((char*)info.address) + (y * info.pitch)
                          + (x * info.pixel_size));
 ...
 */
void v_fb::ega_puts(uint32_t& x, uint32_t& y, uint8_t ega_attributes,
                    const char* string) {
    uint32_t index = 0;
    uint32_t x_cnt = x;
    uint32_t y_cnt = y;
...
inline void* v_fb::get_target_address(uint32_t x, uint32_t y) {
    ...
    return (void*)((uintptr_t)info.address + (y * info.pitch)
                   + (x * info.pixel_size));
}
...
void v_fb::draw_pixel(void* target_address, uint32_t color) {
    ...
    switch (info.depth) {
        case 8: *(uint8_t*)target_address = (uint8_t)(color & 0xff); break;
        ...
        case 16: *(uint16_t*)target_address = (uint16_t)(color & 0xffff); break;
...
void v_fb::draw_line(int x0, int y0, int x1, int y1, uint32_t color) {
    ...
    if (y0 == y1) {
        ...
        for (x0 = x0; x0 <= x1; x0++) {
            draw_pixel(target_address, color);
            target_address
                = (void*)((uintptr_t)target_address + info.pixel_size);
        }
        return;
    }
 ...
 */
void v_fb::draw_rect(uint32_t x_0, uint32_t y_0, uint32_t x_1, uint32_t y_1,
    ...
    for (y_i = (y_0 + border_mod); y_i != y_0; y_i += border_it) {
        for (x_i = x_0; x_i <= x_1; x_i++) { draw_pixel(x_i, y_i, color); }
    }
    ...
    for (x_i = (x_0 + border_mod); x_i != x_0; x_i += border_it) {
        for (y_i = y_0; y_i <= y_1; y_i++) { draw_pixel(x_i, y_i, color); }
    }
 ...
 */
void v_fb::draw_rect_fill(uint32_t x0, uint32_t y0, uint32_t x1, uint32_t y1,
                          uint32_t color) {
 ...
 */
void v_fb::fb_putc(uint32_t& x, uint32_t& y, const char target_char,
    ...
    switch (target_char) {
        ...
        default: {
            ...
            for (int yIt = 0; yIt < char_height; yIt++) {
                ...
                target_address
                    = (void*)((uintptr_t)target_address + line_offset);
            }
 ...
 * @param bg_color      Background character color
 */
void v_fb::fb_puts(uint32_t& x, uint32_t& y, const char* string,
                   uint32_t fg_color, uint32_t bg_color) {
...
void v_fb::fb_blank(uint8_t color) {
    ...
    std_k::memset(info.address, color, info.size);
}
 ...
 */
void v_fb::fb_place_bmp(uint32_t x, uint32_t y, unsigned int bmp,
                        uint32_t length, uint32_t fg, uint32_t bg) {
...
void v_fb::show(uint32_t x, uint32_t y) {
    ...
    unsigned char* target
        = (unsigned char*)fb.address + (fb.pitch * y) + (fb.pixel_size * x);

    if (info.width < fb.width) {

        for (unsigned int row = 0; row < info.height; row++) {

            std_k::memcpy(target, source, info.pitch);
            target += fb.pitch;
            source += info.pitch;
        }
        ...
        std_k::memcpy(target, source, info.size);
    }
}

void v_fb::blank(uint8_t color) { fb_blank(color); }

void v_fb::draw_c(uint32_t& x, uint32_t& y, char target_char, uint32_t fg,
...
void v_fb::draw_s(uint32_t& x, uint32_t& y, const char* string, uint32_t fg,
                  uint32_t bg, uint32_t ega) {
