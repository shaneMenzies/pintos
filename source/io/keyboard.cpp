 ...
 *
 */
constexpr char code_translation[0x100] = {
...
constexpr char shift_translation[0x100] = {
...
inline bool shift_active() { return (made[LEFT_SHIFT] || made[RIGHT_SHIFT]); }

inline bool ctrl_active() { return (made[LEFT_CTRL] || e0_made[RIGHT_CTRL]); }

inline bool alt_active() { return (made[LEFT_ALT] || e0_made[RIGHT_ALT]); }
...
void key_make(unsigned char key) {
    ...
    if (lock[E0]) {
        ...
        if (lock[CAPS] && std_k::is_alphabet(to_write)) {
            if (to_write > 'Z')
                to_write -= 32;
            else
                to_write += 32;
    ...
    if (to_write != 0) { active_terminal->send_key(to_write); }
}
...
void update_lights() {
    ...
    unsigned char light_data
        = (lock[SCROLL] << SCROLL | lock[NUM] << NUM | lock[CAPS] << CAPS);
...
void kb_handler::buffer_hard_clear() {
    ...
    std_k::memset(buffer_start, 0, (size_t)(buffer_end - buffer_start));
    buffer_next = buffer_start;
}
...
void kb_handler::run_action(char target) {
    ...
    switch (key_actions[(unsigned char)target].code) {
        ...
        case TO_BUFFER: buffer_write_c(target); break;
        ...
        case HARD_CLEAR: buffer_hard_clear(); break;

        case CLEAR_BUFFER: buffer_clear(); break;
